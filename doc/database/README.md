...
本地开发和真实后端联调默认使用 `seed.sql` 中的以下账号，初始密码均为 `123456`。

| 端 | 角色 | 登录账号 | 状态 |
| --- | --- | --- | --- |
| 前台用户端 | 增长编导 | `linnan@ai-script.local` | 启用 |
| 后台管理端 | 超级管理员 | `admin@ai-script.local` | 启用 |
| 后台管理端 | 品牌管理员 | `tangyu@ai-script.local` | 启用 |
| 后台管理端 | 审核员 | `zhounan@ai-script.local` | 停用，不能登录 |
