...
const PromptTemplatesPage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await systemApi.getPromptTemplates({ page, pageSize });
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Prompt 模板"
        description="单独管理系统 Prompt、场景编码和版本信息。"
        actions={<div className="toolbar-group"><button className="toolbar-btn" type="button" onClick={() => { setPage(1); if (page === 1) load(); }}><RefreshCcw size={16} />刷新</button><button className="toolbar-btn primary" type="button" onClick={openCreate}><Plus size={16} />新增 Prompt</button></div>}
      />
      ...
      {rows.length ? (
        ...
        <EmptyState title={loading ? '加载中...' : '暂无 Prompt 模板'} description="后端返回空时展示空态，不与其他模块混在一个页面。" icon={<FileText size={22} />} />
      )}
      ...
      <Modal
        ...
        description="可在这里查看和修改系统提示词、用户提示词和响应 Schema。Brief 检测接口使用 sceneCode=brief_detect。"
