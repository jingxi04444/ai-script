...
const menuGroups: Array<{ title: string; items: MenuItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '积分调整', path: '/membership/points', icon: Coins },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '协议管理', path: '/system/legal-documents', icon: ScrollText },
...
const routeMeta: Record<string, { title: string; desc: string }> = {
  ...
  '/membership/points': { title: '积分调整', desc: '人工调整用户积分并记录操作流水。' },
  ...
  '/system/legal-documents': { title: '协议管理', desc: '维护并发布用户协议和隐私政策。' },
